// Universal REST API server for all LLMs
...
// Enable CORS for all LLM platforms
app.use('/*', cors({
  origin: '*',
...
const HUB_URL = process.env.HUB_URL || 'http://localhost:4021';

// Health check
app.get('/health', (c) => c.json({ status: 'ok', service: 'meterx402-connector' }));

// List services (compatible with all LLMs)
...
// Call a service (simplified interface for all LLMs)
...
console.log(`🌐 MeterX402 Universal Connector starting on :${port}`);
