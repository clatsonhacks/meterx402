...
function renderExplore() {
  ...
  if (featured.length > 0 && M.loaded && !M.q) {
    $("mkt-featured").innerHTML = `<div class="featured">
      ...
      <div class="featured-track">${featured.map((l) => `<div class="featured-svc">${cardHtml(l)}</div>`).join("")}</div>
    </div>`;
    $("mkt-featured").querySelectorAll(".svc[data-id]").forEach((card) => {
